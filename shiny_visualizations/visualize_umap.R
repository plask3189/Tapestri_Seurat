...
get_obj <- function() {
  ...
  p <- c("seurat_merged.rds","seurat_obj.rds",
         "/Users/kateplas/Library/Mobile Documents/com~apple~CloudDocs/Documents/Miles_Lab/structure_scDNA_protein/seurat_merged.Rds")
...
join_layers <- function(obj, assay, layer_prefix = "counts") {
  # ── 1. Confirm assay exists ──────────────────────────────────
  av <- tryCatch(as.character(Assays(obj)), error = function(e) names(obj@assays))
  if (!assay %in% av)
    stop("Assay '", assay, "' not found. Available: ", paste(av, collapse=", "))
  
  # ── 2. Enumerate layers — try three APIs in order ────────────
  #   a) Seurat5:  Layers(obj, assay=assay)  — preferred
  #   b) Seurat5:  Layers(assay_obj)         — fallback
  #   c) Seurat4:  names(assay_obj@layers)
  assay_obj  <- obj@assays[[assay]]
  all_layers <- tryCatch(
    as.character(Layers(obj, assay = assay)),
    error = function(e1) tryCatch(
      as.character(Layers(assay_obj)),
      error = function(e2) as.character(names(assay_obj@layers))
    )
  )
  ...
  # ── 3. Pre-compute name lookup from LogMap masks ─────────────
  # Seurat5 split layer matrices always have NULL dimnames.
  # Names live in @features@.Data and @cells@.Data (LogMap slots):
  #   rownames = all feature / cell names across all samples
  #   columns  = layer names; TRUE = this feature/cell belongs to that layer
  all_feat_names <- tryCatch(rownames(assay_obj@features@.Data), error = function(e) NULL)
  all_cell_names <- tryCatch(rownames(assay_obj@cells@.Data),    error = function(e) NULL)
  
  get_names_for_layer <- function(lyr) {
    feats <- tryCatch({
      if (!is.null(all_feat_names) && lyr %in% colnames(assay_obj@features@.Data)) {
        all_feat_names[as.logical(assay_obj@features@.Data[, lyr])]
      } else all_feat_names
    }, error = function(e) all_feat_names)
    
    cells <- tryCatch({
      if (!is.null(all_cell_names) && lyr %in% colnames(assay_obj@cells@.Data)) {
        all_cell_names[as.logical(assay_obj@cells@.Data[, lyr])]
      } else all_cell_names
    }, error = function(e) all_cell_names)
    
    list(feats = feats, cells = cells)
  }
  
  get_layer_mat <- function(lyr) {
    m <- tryCatch(
      as.matrix(LayerData(obj, assay = assay, layer = lyr)),
      error = function(e) stop("LayerData failed for layer '", lyr, "': ", conditionMessage(e))
    )
    nms <- get_names_for_layer(lyr)
    if (is.null(rownames(m)) || (nrow(m) > 0 && all(rownames(m) == ""))) {
      if (!is.null(nms$feats) && length(nms$feats) == nrow(m)) rownames(m) <- nms$feats
    }
    if (is.null(colnames(m)) || (ncol(m) > 0 && all(colnames(m) == ""))) {
      if (!is.null(nms$cells) && length(nms$cells) == ncol(m)) colnames(m) <- nms$cells
    }
    m
  }
  
  if (length(target) == 1) return(get_layer_mat(target[1]))
  
  # ── 4. Multiple split layers — retrieve and merge ────────────
  pieces <- lapply(target, function(lyr) get_layer_mat(lyr))
...
build_base_df <- function(obj) {
  ...
  umap3 <- if (ncol(umap_2d) >= 3) umap_2d[, 3] else {
    if (!is.null(pca_emb) && "PC_3" %in% colnames(pca_emb)) pca_emb[, "PC_3"]
    else rep(0, length(cells))
  }
  data.frame(
    cell    = cells,
    umap_1  = umap_2d[, 1],
...
ui <- fluidPage(
  ...
  tabsetPanel(id="main_tabs",
                       ...
                       div(class="dual-layout",
                           div(class="dual-sidebar",
                               ...
                               div(class="cs",
                                   ...
                                   div(class="scale-row",
                                       div(class="scale-label","low"),div(class="scale-bar"),
              ...
              padding:3px 0;border-radius:4px;")
                               ...
                               div(class="cs",div(class="cs-title","Filter by Sample"),uiOutput("sb_sample_ui")),
                               div(class="cs",div(class="cs-title","Filter by Cluster"),uiOutput("sb_cluster_ui")),
                               div(class="cs",div(class="cs-title","NGT Stats"),verbatimTextOutput("sb_stats"))
                           ),
                           div(class="dual-plots",
                               div(class="plot-half",div(class="plot-title","DSB Protein"),
                                   plotlyOutput("left_plot",width="100%",height="100%")),
                               div(class="plot-half",div(class="plot-title","NGT Variant"),
                           ...
                           div(class="analytics-sidebar",
                               div(class="cs",
                                   ...
                                   selectInput("comut_metric","",
                                               choices=c("Co-occurring cells"="co_occur","Jaccard similarity"="jaccard",
                                                         "Obs - Expected (mutex)"="mutex"), selected="co_occur")
                               ),
                               div(class="cs",
              ...
              tabPanel("Protein \u00d7 NGT",
                       div(class="analytics-layout",
                           div(class="analytics-sidebar",
                               ...
                               div(class="cs",div(class="cs-title","Sample Filter"),uiOutput("pxn_sample_ui")),
                               div(class="cs",
                               ...
                               div(class="cs",div(class="cs-title","Stats"),verbatimTextOutput("pxn_stats"))
                           ),
                           div(class="analytics-main",
                               div(class="analytics-row",
                                   div(class="analytics-card",style="flex:1;",
...
server <- function(input, output, session) {
  ...
  var_avail <- reactive({
    vsm    <- var_sample_map(); n_samp <- length(vsm)
  ...
  dsb_feat_range <- reactive({
    feat <- req(input$left_feat); mat <- dsb_mat()
    req(feat %in% rownames(mat))
    vals <- as.numeric(mat[feat, ])
    list(lo = floor(quantile(vals,.01,na.rm=TRUE)*10)/10,
         hi = ceiling(quantile(vals,.99,na.rm=TRUE)*10)/10)
  })
  ...
  observeEvent(input$dsb_range, {
    ...
    if (!is.null(input$dsb_range) &&
        (abs(input$dsb_range[1]-rng$lo)>0.05 || abs(input$dsb_range[2]-rng$hi)>0.05))
      dsb_user_range(input$dsb_range)
  }, ignoreInit=TRUE)
  
  output$dsb_range_ui <- renderUI({
    rng <- dsb_feat_range(); cur <- dsb_user_range() %||% c(rng$lo, rng$hi)
    sliderInput("dsb_range", NULL,
                min=min(rng$lo,cur[1])-1, max=max(rng$hi,cur[2])+1,
                value=cur, step=0.1, ticks=FALSE)
  })
  
  dsb_clamp <- reactive({
    input$dsb_range %||% { rng <- dsb_feat_range(); c(rng$lo, rng$hi) }
  })
  ...
  observe({
    dsb <- sort(rownames(dsb_mat())); ngt <- sort(rownames(ngt_mat()))
  ...
  output$right_feat_availability <- renderUI({ avail_badge(input$right_feat, var_avail()) })
  output$pxn_variant_avail       <- renderUI({ avail_badge(input$pxn_variant, var_avail()) })
  ...
  z_label <- reactive({
    emb <- tryCatch(Embeddings(obj(), "umap"), error = function(e) NULL)
    if (!is.null(emb) && ncol(emb) >= 3) "UMAP 3" else "PC 3"
  })
  
  output$left_plot <- renderPlotly({
    df    <- filt_df(); feat <- req(input$left_feat)
    mat   <- dsb_mat(); req(feat %in% rownames(mat))
    clamp <- dsb_clamp()
  ...
  comut_cells <- reactive({
    bdf <- base_df(); sels <- input$comut_samples %||% unique(bdf$sample)
    intersect(bdf$cell[bdf$sample %in% sels], colnames(ngt_mat()))
  })
  
  top_vars <- reactive({
    cells <- comut_cells(); mat <- ngt_mat()
    vmeta <- tryCatch(obj()@assays$NGT@meta.data, error=function(e) data.frame())
  ...
  comut_mat_data <- reactive({
    cells <- comut_cells(); mat <- ngt_mat(); vars <- top_vars()
    excl  <- isTRUE(input$comut_excl_missing); met <- input$comut_metric %||% "co_occur"
  ...
  output$comut_heatmap <- renderPlotly({
    cm <- comut_mat_data(); vars <- rownames(cm)
    show <- isTRUE(input$comut_showvals); fs <- input$comut_fontsize %||% 8
    met  <- input$comut_metric %||% "co_occur"
    lbl  <- c(co_occur="Co-occurring cells",jaccard="Jaccard",mutex="Obs-Exp")[met]
    cs   <- if(met=="mutex") list(c(0,"#D6604D"),c(0.5,"#0d1117"),c(1,"#2166AC"))
    else list(c(0,"#0d1117"),c(0.3,"#1a4a7a"),c(0.6,"#2166AC"),c(1,"#d73027"))
    plot_ly(x=vars,y=vars,z=cm,type="heatmap",colorscale=cs,
            ...
            colorbar=list(title=list(text=lbl,font=list(color="#8b949e",size=10)),
                          tickfont=list(color="#8b949e",size=9),
                 ...
                 margin=list(l=130,r=20,t=20,b=130)) %>% config(displaylogo=FALSE)
  })
  ...
  output$comut_freq <- renderPlotly({
    ...
    rows  <- lapply(samps, function(s) {
      ...
      data.frame(variant=vars, pct=rowMeans(sub==1|sub==2,na.rm=TRUE)*100,
                 sample=s, stringsAsFactors=FALSE)
    })
    df <- do.call(rbind, Filter(Negate(is.null), rows))
    df$variant <- factor(df$variant, levels=rev(top_vars()))
    samp_pal <- setNames(colorRampPalette(c("#2166AC","#4dac26","#d73027","#f4a40e","#6A4C93"))(length(samps)),samps)
    p <- plot_ly(type="bar")
    for (s in samps) {
      sub <- df[df$sample==s,]
      p   <- add_trace(p,x=sub$pct,y=sub$variant,name=s,type="bar",orientation="h",
                       marker=list(color=samp_pal[s]),
                       hovertemplate=paste0("<b>",s,"</b><br>%{y}<br>%{x:.1f}%<extra></extra>"))
    }
    p %>% layout(barmode="stack",paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
                 font=list(color="#e6edf3",family="IBM Plex Mono"),
                 ...
                 margin=list(l=140,r=20,t=20,b=50)) %>% config(displaylogo=FALSE)
  })
  
  output$comut_cluster_bar <- renderPlotly({
    bdf  <- base_df(); sels <- input$comut_samples %||% unique(bdf$sample)
    bdf  <- bdf[bdf$sample %in% sels,]; mat <- ngt_mat()
    vars <- top_vars()[seq_len(min(10,length(top_vars())))]
    excl <- isTRUE(input$comut_excl_missing)
    rows <- lapply(vars, function(v) {
      raw <- mat[v, bdf$cell]; mut <- if(excl) raw==1|raw==2 else raw==1|raw==2|raw==3
      mut[is.na(mut)] <- FALSE
      data.frame(variant=v,cluster=bdf$cluster,mutated=mut,stringsAsFactors=FALSE)
    })
    df  <- do.call(rbind, rows)
    agg <- df %>% group_by(variant,cluster) %>% summarise(pct=mean(mutated)*100,.groups="drop")
    cl_ord <- sort(unique(agg$cluster)); p <- plot_ly(type="bar")
    for (cl in cl_ord) {
    ...
    p %>% layout(barmode="group",paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
                 font=list(color="#e6edf3",family="IBM Plex Mono"),
                 ...
                 yaxis=list(title="% mutated",tickfont=list(size=9,color="#8b949e"),
                            gridcolor="#1e2530",zeroline=FALSE),
                 legend=list(bgcolor="#161b22",bordercolor="#30363d",borderwidth=1,font=list(size=10)),
                 margin=list(l=60,r=20,t=10,b=80)) %>% config(displaylogo=FALSE)
  })
  
  # ── TAB 3: PROTEIN × NGT ────────────────────────────────────
  
  pxn_base <- reactive({
    bdf <- base_df(); sels <- input$pxn_samples %||% unique(bdf$sample)
    bdf[bdf$sample %in% sels,]
  })
  
  pxn_df <- reactive({
    bdf <- pxn_base(); prot <- req(input$pxn_protein); vari <- req(input$pxn_variant)
    mat_d <- dsb_mat(); mat_n <- ngt_mat(); req(prot %in% rownames(mat_d))
    raw_ngt <- if (vari %in% rownames(mat_n)) mat_n[vari, bdf$cell] else rep(NA_real_,nrow(bdf))
    geno <- ifelse(is.na(raw_ngt),"N/A",ifelse(raw_ngt==0,"WT (0)",
                                               ifelse(raw_ngt==1,"Het (1)",ifelse(raw_ngt==2,"Hom (2)","Missing (3)"))))
    data.frame(cell=bdf$cell,cluster=bdf$cluster,sample=bdf$sample,
               protein=as.numeric(mat_d[prot,bdf$cell]),genotype=geno,stringsAsFactors=FALSE)
  })
  ...
  output$pxn_violin <- renderPlotly({
    df <- pxn_filt(); prot <- input$pxn_protein
    genos <- intersect(c("WT (0)","Het (1)","Hom (2)"), unique(df$genotype))
    p <- plot_ly(type="violin")
    for (g in genos) {
      sub <- df[df$genotype==g,]; col <- NGT_COLORS[g] %||% "#aaa"
    ...
    p %>% layout(paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
                 ...
                 yaxis=list(title=prot,tickfont=list(size=9,color="#8b949e"),
                            gridcolor="#1e2530",zeroline=FALSE),
                 legend=list(bgcolor="#161b22",bordercolor="#30363d",borderwidth=1,font=list(size=10)),
                 violinmode="group",margin=list(l=60,r=20,t=20,b=50)) %>% config(displaylogo=FALSE)
  })
  
  output$pxn_heatmap <- renderPlotly({
    bdf <- pxn_base(); prot <- req(input$pxn_protein)
    mat_d <- dsb_mat(); mat_n <- ngt_mat(); req(prot %in% rownames(mat_d))
    freq  <- rowMeans(mat_n[,bdf$cell,drop=FALSE]==1|mat_n[,bdf$cell,drop=FALSE]==2,na.rm=TRUE)
    ...
    rows  <- lapply(top20, function(v) {
      ...
      geno <- ifelse(is.na(raw),"N/A",ifelse(raw==0,"WT (0)",ifelse(raw==1,"Het (1)",
                                                                    ifelse(raw==2,"Hom (2)","Missing (3)"))))
      data.frame(variant=v,genotype=geno,protein=pv,stringsAsFactors=FALSE)
    })
    df2  <- do.call(rbind,rows); df2 <- df2[!df2$genotype %in% c("Missing (3)","N/A"),]
    agg  <- df2 %>% group_by(variant,genotype) %>% summarise(med=median(protein,na.rm=TRUE),.groups="drop")
    ...
    plot_ly(x=gl,y=top20,z=zm,type="heatmap",
            ...
            colorbar=list(title=list(text=paste("Median",prot),font=list(color="#8b949e",size=10)),
                          tickfont=list(color="#8b949e",size=9),
                          bgcolor="#161b22",bordercolor="#30363d",borderwidth=1)
    ) %>% layout(paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
                 font=list(color="#e6edf3",family="IBM Plex Mono"),
                 ...
                 margin=list(l=140,r=20,t=20,b=60)) %>% config(displaylogo=FALSE)
  })
  
  output$pxn_scatter <- renderPlotly({
    bdf <- pxn_base(); prot2 <- req(input$pxn_prot2); prot3 <- req(input$pxn_prot3)
    vari <- input$pxn_variant; by <- input$pxn_color_by %||% "ngt"
    mat_d <- dsb_mat(); mat_n <- ngt_mat()
    req(prot2 %in% rownames(mat_d), prot3 %in% rownames(mat_d))
    xv <- as.numeric(mat_d[prot2,bdf$cell]); yv <- as.numeric(mat_d[prot3,bdf$cell])
    if (by=="ngt" && !is.null(vari) && nchar(vari)>0) {
      raw <- if(vari %in% rownames(mat_n)) mat_n[vari,bdf$cell] else rep(NA,nrow(bdf))
      cv  <- ifelse(is.na(raw),"N/A",ifelse(raw==0,"WT (0)",ifelse(raw==1,"Het (1)",
                                                                   ifelse(raw==2,"Hom (2)","Missing (3)"))))
      pal <- NGT_COLORS; ttl <- vari
    } else if (by=="sample") {
      cv <- bdf$sample; samps <- sort(unique(cv))
      pal <- setNames(colorRampPalette(c("#E63946","#2A9D8F","#E9C46A","#457B9D","#6A4C93"))(length(samps)),samps)
      ttl <- "Sample"
    } else { cv <- bdf$cluster; pal <- CLUSTER_COLORS; ttl <- "Cluster" }
    ...
    p %>% layout(paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
                 ...
                 margin=list(l=60,r=20,t=20,b=50)) %>% config(displaylogo=FALSE)
  })
  ...
  output$pxn_mutload <- renderPlotly({
    ...
    mat_d <- dsb_mat(); mat_n <- ngt_mat(); req(prot %in% rownames(mat_d))
    pv   <- as.numeric(mat_d[prot,bdf$cell])
    load <- colSums(mat_n[,bdf$cell,drop=FALSE]==1|mat_n[,bdf$cell,drop=FALSE]==2,na.rm=TRUE)
    df   <- data.frame(load_bin=factor(pmin(load,8L)),protein=pv,sample=bdf$sample)
    plot_ly(df,x=~load_bin,y=~protein,type="box",color=~load_bin,colors=viridis(9),
            hovertemplate="Load %{x}<br>%{y:.2f}<extra></extra>",showlegend=FALSE
    ) %>% layout(paper_bgcolor="#0d1117",plot_bgcolor="#0d1117",
                 font=list(color="#e6edf3",family="IBM Plex Mono"),
  ...
  output$pxn_stats <- renderText({
    df <- pxn_filt(); prot <- input$pxn_protein %||% ""; vari <- input$pxn_variant %||% ""
